...
export interface ImportDataCaregiver {
...
export class DataTransformer1 extends DataTransformer {
  static transform(row: CSVRow): ImportDataCaregiver { 
    return {
      franchisor_id: this.cleanString(row.franchisor_id),
      agency_id: this.cleanString(row.agency_id),
      subdomain: this.cleanString(row.subdomain),
      ...
      location_name: this.cleanString(row.location_name),
